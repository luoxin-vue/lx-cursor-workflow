...
- 公司/项目本地规则、编码规范、公共组件、模板页面、测试构建和发布约定优先于通用工作流与编码建议；可兼容时组合执行，无法协调时在计划阶段显式报告冲突，不静默覆盖。仓库只固化优先级和冲突处理方法，不把单一公司的具体规则推广到所有项目。
- 提供 `lx-bugfix` 作为已有行为错误、回归、异常、性能退化和测试失败的专用入口；新能力仍从 `lx-grill` 开始。
- Bug 修复采用证据驱动的 `fast`、`standard`、`deep` 分级：简单参数/字段/单函数问题默认快速处理，接口边界不因存在调用关系自动升级；只有契约未知、真实响应矛盾或高风险因素出现时才加深流程。
